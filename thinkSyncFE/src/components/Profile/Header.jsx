...
const ProfileHeader = ({
  ...
  const coverImage =
    user?.details?.coverImage || "https://placehold.co/800x200";
  ...
  return (
    <div className="bg-white/80 dark:bg-gray-800/90 backdrop-blur-xl rounded-2xl sm:rounded-3xl border border-gray-200/50 dark:border-gray-700/50 overflow-hidden mb-6 sm:mb-8 shadow-xl sm:shadow-2xl shadow-blue-500/5 dark:shadow-blue-400/5 relative">
      ...
      {/* Avatar & Info */}
      <div className="relative px-4 pb-6 sm:px-8 sm:pb-10 -mt-16 sm:-mt-20">
        <div className="relative z-10 rounded-3xl bg-white/85 dark:bg-gray-900/80 backdrop-blur-2xl shadow-2xl border border-white/60 dark:border-gray-700/40 px-4 py-6 sm:px-8 sm:py-8">
          <div className="flex flex-col sm:flex-row sm:items-end gap-4 sm:gap-8">
          {/* Avatar */}
            <div className="relative self-center sm:self-auto -mt-10 sm:-mt-14">
              <img
                src={avatarUrl}
                alt="Profile"
                className="w-24 h-24 sm:w-32 sm:h-32 md:w-36 md:h-36 rounded-2xl sm:rounded-3xl border-4 border-white dark:border-gray-800 shadow-2xl transition-transform duration-500 hover:scale-105 object-cover"
              />
            </div>

          {/* User Info */}
            <div className="flex-1 pt-4 sm:pt-6 text-center sm:text-left">
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 dark:text-gray-100 drop-shadow-sm">
              {user.displayName}
              </h1>
              <p className="text-gray-600 dark:text-gray-400 mb-3 font-medium text-sm sm:text-base">
              @{user.username}
              </p>
              {user.details?.bio && (
                <p className="text-gray-700 dark:text-gray-300 mb-5 sm:mb-7 leading-relaxed text-sm sm:text-base">
                {user.details.bio}
                </p>
              )}
